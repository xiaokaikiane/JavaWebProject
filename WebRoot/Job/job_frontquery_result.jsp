	...
	<div class="container">
		...
		<div class="row">
				...
				<div>
					...
					<ul class="nav nav-tabs" role="tablist">
						...
						<li role="presentation" class="active"><a
							href="#jobListPanel" aria-controls="jobListPanel" role="tab"
							data-toggle="tab">职位列表</a></li>
						<li role="presentation"><a
							href="<%=basePath %>Job/job_frontAdd.jsp" style="display: none;">添加职位</a></li>
					</ul>
					<!-- Tab panes -->
										...
										<table class="table table-condensed table-hover">
 										...
 										%>
											<tr>
												...
												<td><a
													href="<%=basePath  %>Job/<%=job.getJobId() %>/frontshow"><i
														class="fa fa-info"></i>&nbsp;查看</a>&nbsp; <a href="#"
													onclick="jobEdit('<%=job.getJobId() %>');"
													style="display: none;"><i class="fa fa-pencil fa-fw"></i>编辑</a>&nbsp;
													<a href="#" onclick="jobDelete('<%=job.getJobId() %>');"
													style="display: none;"><i class="fa fa-trash-o fa-fw"></i>删除</a>
												</td>
											</tr>
											<%}%>
										</table>
									...
									<nav class="pull-left">
										<ul class="pagination">
											...
											<li class="<%= currentPage==i?"active":"" %>"><a
												href="#" onclick="GoToPage(<%=i %>,<%=totalPage %>);"><%=i %></a></li>
											<%  } %>
											<li><a href="#"
			...
			<div class="col-md-3 wow fadeInRight">
				...
				<form name="jobQueryForm" id="jobQueryForm"
					action="<%=basePath %>Job/frontlist" class="mar_t15" method="post">
					<div class="form-group">
						<label for="jobTypeObj_jobTypeId">职位类别：</label> <select
							id="jobTypeObj_jobTypeId" name="jobTypeObj.jobTypeId"
							class="form-control">
					...
					<div class="form-group">
						<label for="jobName">职位名称:</label> <input type="text" id="jobName"
							name="jobName" value="<%=jobName %>" class="form-control"
							placeholder="请输入职位名称">
					</div>
					...
					<div class="form-group">
						<label for="xlyq">学历要求:</label> <input type="text" id="xlyq"
							name="xlyq" value="<%=xlyq %>" class="form-control"
							placeholder="请输入学历要求">
					</div>
					...
					<div class="form-group">
						<label for="gzqy">工作区域:</label> <input type="text" id="gzqy"
							name="gzqy" value="<%=gzqy %>" class="form-control"
							placeholder="请输入工作区域">
					</div>
					...
					<div class="form-group">
						<label for="companyObj_companyUserName">发布企业：</label> <select
							id="companyObj_companyUserName" name="companyObj.companyUserName"
							class="form-control">
 					...
 					if(companyObj!=null && companyObj.getCompanyUserName()!=null && companyObj.getCompanyUserName().equals(companyTemp.getCompanyUserName()))
 						selected = "selected";
	 				%>
							<option value="<%=companyTemp.getCompanyUserName() %>"
								<%=selected %>><%=companyTemp.getCompanyName() %></option>
							<%
	 				}
	 				%>
						</select>
					...
					<div class="form-group">
						<label for="addTime">发布时间:</label> <input type="text" id="addTime"
							name="addTime" class="form-control" placeholder="请选择发布时间"
							value="<%=addTime %>" onclick="SelectDate(this,'yyyy-MM-dd')" />
					</div>
			...
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">
						<i class="fa fa-edit"></i>&nbsp;职位信息编辑
					</h4>
				</div>
				<div class="modal-body" style="height: 450px; overflow: scroll;">
					<form class="form-horizontal" name="jobEditForm" id="jobEditForm"
						enctype="multipart/form-data" method="post" class="mar_t15">
						<div class="form-group">
							<label for="job_jobId_edit" class="col-md-3 text-right">职位id:</label>
							<div class="col-md-9">
								<input type="text" id="job_jobId_edit" name="job.jobId"
									class="form-control" placeholder="请输入职位id" readOnly>
							</div>
						</div>
						<div class="form-group">
							<label for="job_jobTypeObj_jobTypeId_edit"
								class="col-md-3 text-right">职位类别:</label>
							<div class="col-md-9">
								<select id="job_jobTypeObj_jobTypeId_edit"
									name="job.jobTypeObj.jobTypeId" class="form-control">
								</select>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-md-9">
								<input type="text" id="job_jobName_edit" name="job.jobName"
									class="form-control" placeholder="请输入职位名称">
							</div>
						</div>
						<div class="form-group">
							<label for="job_jobDesc_edit" class="col-md-3 text-right">职位描述:</label>
							<div class="col-md-9">
								<textarea name="job.jobDesc" id="job_jobDesc_edit"
						...
						<div class="form-group">
							...
							<div class="col-md-9">
								<input type="text" id="job_salary_edit" name="job.salary"
									class="form-control" placeholder="请输入工作薪酬">
							</div>
						</div>
						<div class="form-group">
							<label for="job_zprs_edit" class="col-md-3 text-right">招聘人数:</label>
							<div class="col-md-9">
								<input type="text" id="job_zprs_edit" name="job.zprs"
									class="form-control" placeholder="请输入招聘人数">
							</div>
						</div>
						<div class="form-group">
							<label for="job_xlyq_edit" class="col-md-3 text-right">学历要求:</label>
							<div class="col-md-9">
								<input type="text" id="job_xlyq_edit" name="job.xlyq"
									class="form-control" placeholder="请输入学历要求">
							</div>
						</div>
						<div class="form-group">
							<label for="job_yxqx_edit" class="col-md-3 text-right">有效期限:</label>
							<div class="col-md-9">
								<input type="text" id="job_yxqx_edit" name="job.yxqx"
									class="form-control" placeholder="请输入有效期限">
							</div>
						</div>
						<div class="form-group">
							<label for="job_gzqy_edit" class="col-md-3 text-right">工作区域:</label>
							<div class="col-md-9">
								<input type="text" id="job_gzqy_edit" name="job.gzqy"
									class="form-control" placeholder="请输入工作区域">
							</div>
						</div>
						<div class="form-group">
							<label for="job_gzdz_edit" class="col-md-3 text-right">工作地址:</label>
							<div class="col-md-9">
								<input type="text" id="job_gzdz_edit" name="job.gzdz"
									class="form-control" placeholder="请输入工作地址">
							</div>
						</div>
						<div class="form-group">
							<label for="job_viewNum_edit" class="col-md-3 text-right">浏览次数:</label>
							<div class="col-md-9">
								<input type="text" id="job_viewNum_edit" name="job.viewNum"
									class="form-control" placeholder="请输入浏览次数">
							</div>
						</div>
						<div class="form-group">
							<label for="job_companyObj_companyUserName_edit"
								class="col-md-3 text-right">发布企业:</label>
							<div class="col-md-9">
								<select id="job_companyObj_companyUserName_edit"
									name="job.companyObj.companyUserName" class="form-control">
								</select>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-md-9">
								<div class="input-group date job_addTime_edit col-md-12"
									data-link-field="job_addTime_edit">
									<input class="form-control" id="job_addTime_edit"
										name="job.addTime" size="16" type="text" value=""
										placeholder="请选择发布时间" readonly> <span
	...
	margin-bottom: 5px;
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary"
						onclick="ajaxJobModify();">提交</button>
				</div>
			</div>
			<!-- /.modal-content -->
		</div>
